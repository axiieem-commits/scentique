...
app.post("/api/auth/reset-password", async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      "SELECT id FROM users WHERE role = 'customer' AND LOWER(name) = LOWER(?) AND email = ? LIMIT 1",
      [username, email]
    );

    if (rows.length === 0) {
      return res.status(404).json({ message: "No customer account matches that username and email." });
    }
...
async function requireCustomerAccount(userId, queryRunner = db) {
...
async function getOrCreateCart(userId, queryRunner = db) {
...
async function loadCartItems(userId, queryRunner = db) {
  const [rows] = await queryRunner.query(
    `SELECT
        ...
        CASE
          WHEN products.sale = TRUE AND products.sale_price IS NOT NULL THEN products.sale_price
          ELSE products.price
        END AS price,
        products.stock,
...
app.get("/api/cart", async (req, res) => {
  try {
    ...
    const customer = await requireCustomerAccount(userId);

    if (!customer) {
      return res.status(401).json({ message: "Please login with a valid customer account." });
    }
...
app.post("/api/cart/items", async (req, res) => {
  ...
  try {
    ...
    const quantityToAdd = normalizeWholeNumber(req.body.quantity ?? req.body.qty ?? 1);

    if (!productId || quantityToAdd === null || quantityToAdd <= 0) {
      return res.status(400).json({ message: "Choose a valid product and quantity." });
    }

    await connection.beginTransaction();

    const customer = await requireCustomerAccount(userId, connection);
    if (!customer) {
    ...
    const [productRows] = await connection.query(
      `SELECT id, name, stock,
        CASE WHEN sale = TRUE AND sale_price IS NOT NULL THEN sale_price ELSE price END AS active_price
       FROM products
       WHERE id = ?
       LIMIT 1
       FOR UPDATE`,
    ...
    if (stock <= 0) {
      await connection.rollback();
      return res.status(400).json({ message: "This perfume is out of stock." });
    }

    const cartId = await getOrCreateCart(userId, connection);
    const [existingRows] = await connection.query(
      "SELECT cart_item_id, quantity FROM cart_items WHERE cart_id = ? AND product_id = ? LIMIT 1 FOR UPDATE",
      [cartId, productId]
    );
    ...
    if (nextQuantity > stock) {
      await connection.rollback();
      return res.status(400).json({ message: `Only ${stock} ${product.name} item(s) available in stock.` });
    }

    const subtotal = Number(product.active_price || 0) * nextQuantity;

    if (existingRows.length > 0) {
      await connection.query(
        "UPDATE cart_items SET quantity = ?, subtotal = ? WHERE cart_item_id = ?",
        [nextQuantity, subtotal, existingRows[0].cart_item_id]
      );
    } else {
      await connection.query(
        "INSERT INTO cart_items (cart_id, product_id, quantity, subtotal) VALUES (?, ?, ?, ?)",
        [cartId, productId, nextQuantity, subtotal]
      );
    }

    const items = await loadCartItems(userId, connection);
...
app.patch("/api/cart/items/:productId", async (req, res) => {
  ...
  try {
    ...
    const quantity = normalizeWholeNumber(req.body.quantity ?? req.body.qty);

    if (!productId || quantity === null || quantity < 0) {
      return res.status(400).json({ message: "Enter a valid cart quantity." });
    }
    ...
    if (quantity === 0) {
      ...
      const items = await loadCartItems(userId, connection);
      await connection.commit();
      return res.json({ message: "Cart item removed.", items });
    }
    ...
    const [existingRows] = await connection.query(
      "SELECT cart_item_id FROM cart_items WHERE cart_id = ? AND product_id = ? LIMIT 1 FOR UPDATE",
      [cartId, productId]
    );
    const subtotal = Number(product.active_price || 0) * quantity;

    if (existingRows.length > 0) {
      await connection.query(
        "UPDATE cart_items SET quantity = ?, subtotal = ? WHERE cart_item_id = ?",
        [quantity, subtotal, existingRows[0].cart_item_id]
      );
    } else {
      await connection.query(
        "INSERT INTO cart_items (cart_id, product_id, quantity, subtotal) VALUES (?, ?, ?, ?)",
...
app.delete("/api/cart/items/:productId", async (req, res) => {
  try {
    ...
    const items = await loadCartItems(userId);
    res.json({ message: "Cart item removed.", items });
  } catch (error) {
